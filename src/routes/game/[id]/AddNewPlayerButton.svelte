<script lang="ts">
    ...
    function submit() {
        if (!nameInput || !seedInput) return;
        const player = {name: nameInput.value, seed: seedInput.value || undefined};
        try {
    ...
    <Dialog.Content class="sm:max-w-[425px]">
        ...
        <div class="grid gap-4 py-4">
            ...
            <div class="grid grid-cols-4 items-center gap-4">
                ...
                <Input id="seed" placeholder="Leave blank for random" class="col-span-3" />
            </div>
        </div>
        <Dialog.Footer>
